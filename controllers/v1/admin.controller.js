...
module.exports.login = async (req, res, next) => {
    try {
        ...
        if (user && await utils.comparePassword(user.password, password)) {
            ...
            await userSchema.updateOne({ _id: storeUser._id }, { deviceToken })
            return res.status(responseStatus.success).json(utils.successResponse(messages.loggedIn, { ...storeUser ,role:"User", token: utils.SIGNJWT({ id: storeUser._id, password: storeUser.password, deviceToken }) }))
        }
